...
#             if name in ['1bteA00', '1byiA00','1c1yB00']:
    ...
    def get_calpha_graph(self, rec, c_alpha_coords, n_coords, c_coords, coords,cdr=None, cdrh1 = None, cdrh2 = None, cdrh3 = None, cdrl1 = None, cdrl2 = None, cdrl3 = None, isab = None, chain_idlist = None,struc_2nd_res = None):
        cdr_all = []
        for i in cdr:
            cdr_all.extend(i)
        cdr_all = torch.tensor(cdr_all)
        cdrh1_all = []
        for i in cdrh1:
            cdrh1_all.extend(i)
        cdrh1_all = torch.tensor(cdrh1_all)
        cdrh2_all = []
        for i in cdrh2:
            cdrh2_all.extend(i)

        cdrh2_all = torch.tensor(cdrh2_all)
        cdrh3_all = []
        for i in cdrh3:
            cdrh3_all.extend(i)
        cdrh3_all = torch.tensor(cdrh3_all)
        cdrl1_all = []
        for i in cdrl1:
            cdrl1_all.extend(i)
        cdrl1_all = torch.tensor(cdrl1_all)
        cdrl2_all = []
        for i in cdrl2:
            cdrl2_all.extend(i)
        cdrl2_all = torch.tensor(cdrl2_all)
        cdrl3_all = []
        for i in cdrl3:
            cdrl3_all.extend(i)
        cdrl3_all = torch.tensor(cdrl3_all)
        isab_all = []
        for i in isab:
            isab_all.extend(i)
        isab_all = torch.tensor(isab_all)

        chainid_all = []
        chain_count = 0
        for i,chain in enumerate(chain_idlist):
            chainid_all.extend(chain)
            chain_count += 1
        # chainid_all = torch.tensor(chainid_all)
        chainid_all = torch.tensor(list(map(ord, chainid_all)))
    ...
    def get_chain_ids(self, pdb_file):
        heavy_chains = []
        light_chains = []
        with open(pdb_file, 'r') as file:
            for line in file:
                if line.startswith('REMARK'):
                    parts = line.split()
                    for part in parts:
                        if part.startswith('HCHAIN='):
                            heavy_chains.append(part.split('=')[1])
                        elif part.startswith('LCHAIN='):
                            light_chains.append(part.split('=')[1])
        return heavy_chains, light_chains


    def get_receptor_inference(self, rec_path,ID = None):
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=PDBConstructionWarning)
            structure = self.biopython_parser.get_structure(
                'random_id', rec_path)
            rec = structure[0]
        heavy_chains, light_chains = self.get_chain_ids(rec_path)
        coords = []
        c_alpha_coords = []
        n_coords = []
        c_coords = []
        ...
        cdr_region = list(range(27, 39)) + list(range(56, 66)) + list(range(105, 118))
        antibody_chain_ids = heavy_chains + light_chains
        ...
        for i, chain in enumerate(rec):
            ...
            for res_idx, residue in enumerate(chain):
                ...
                if c_alpha != None and n != None and c != None:
                    ...
                    if chain.get_id() in antibody_chain_ids:
                        if residue.id[1] in cdr_region:
                            cdr.append(True)
                        else:
                            cdr.append(False)
                    ...
                    if chain.get_id() in heavy_chains:
                        if residue.id[1] in cdr1:
                            cdrh1.append(True)
                        else:
                            cdrh1.append(False)
                        if residue.id[1] in cdr2:
                            cdrh2.append(True)
                        else:
                            cdrh2.append(False)
                        if residue.id[1] in cdr3:
                            cdrh3.append(True)
                        else:
                            cdrh3.append(False)
                    else:
                    ...
                    if chain.get_id() in light_chains:
                        if residue.id[1] in cdr1:
                            cdrl1.append(True)
                        else:
                            cdrl1.append(False)
                    ...
                    if chain.get_id() in heavy_chains or  chain.get_id() in light_chains:
                        ab.append(True)
                    else:
                        ab.append(False)
                else:
        ...
        for i, chain in enumerate(rec):
            if chain.get_id() in valid_chain_ids:
                ...
                # if chain.get_id() in antibody_chain_ids:
                #     cdr_ = self.create_cdr_mask(lengths[i], [(26, 38), (55, 65), (104, 117)])
                # else:
                #     cdr_ = ''.join(['0' for i in range(lengths[i])])
                # cdr.append(cdr_)
        ...
        return rec, coords, c_alpha_coords, n_coords, c_coords,CDR,CDRH1,CDRH2,CDRH3,CDRL1,CDRL2,CDRL3,IS_AB,CHAIN

    def create_cdr_mask(self,length, cdr_ranges):
        cdr_mask = ['0' for _ in range(length)]
        for start, end in cdr_ranges:
            for i in range(start - 1, end):  # Adjust for 0-based indexing
                cdr_mask[i] = '1'
        return ''.join(cdr_mask)
