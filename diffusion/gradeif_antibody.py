...
class GraDe_IF(nn.Module):
    ...
    def sample_p_zs_given_zt(self,t,s,zt,data,cond,diverse,step, last_step, topk=None):
        """
        sample zs~p(zs|zt)
        """
        ...
        if last_step:
            pred_X = F.softmax(pred,dim = -1)
            if topk is not None:
                wd_idx = data.x.to(bool)
                topk_idx = torch.topk(pred_X.max(axis=1)[0] - pred_X[wd_idx], topk)[1]
                mask = torch.ones(pred_X.shape[0], dtype=bool)
                mask[topk_idx] = False
                pred_X[mask] = data.x[mask]
        ...
        if self.alpha > 0 and step:
            ...
            prob_X_unnormalized[torch.sum(prob_X_unnormalized, dim=-1) == 0] = 1e-5
    ...
    def ddim_sample(self,data,cond = False,diverse=False,stop = 0,step=50, topk=None):
        limit_dist = torch.ones(20)/20
        zt = self.sample_discrete_feature_noise(limit_dist = limit_dist,num_node = data.x.shape[0]) #[N,20] one hot 
        zt = zt.to(data.x.device)
        for s_int in list(reversed(range(stop, self.timesteps,step))): #500
        # for s_int in tqdm(list(reversed(range(stop, self.timesteps,step)))):
            #z_t-1 ~p(z_t-1|z_t),
            s_array = s_int * torch.ones((data.batch[-1]+1, 1)).type_as(data.x)
